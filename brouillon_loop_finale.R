...
for (season in seasons){
  low_bias_points = quantile(abs(get(paste(domaine,"_biais",sep=""))),quant_nodes,na.rm = TRUE)
  ...
  # diff = matrix(mat_model2_low_bias-mat_model1_low_bias,ncol=1)
  # 
  # for(i in 1:length(overlap)){
  #   if((i %in% source_test_indices_eroded) & (i %in% sink_test_indices_eroded)){
  #     if(diff[i] < 0){
  ...
  # diff = matrix(gc_test_2gcm-(mat_model3_low_bias),ncol=1)
  # 
  # for(i in 1:length(overlap)){
  #   if((i %in% source_test_indices_eroded) & (i %in% sink_test_indices_eroded)){
  #     if(diff[i] < 0){
  ...
  # setwd("/home/users/mgarvik/GraphCUT/multivarie/plot")
  save(gc_test_3gcm, file = paste("matrice_graphcut_Q30",season,"_","mpi_cnrm__ipsl","_","eur",".RData",sep = ""))
  pdf(paste("Q30","_","multi","_","mean","_",season,"_","mpi_cnrm__ipsl","_",domaine,".pdf",sep = ""),width = 10,height = 15)
  # model_names <- 
    image.plot(longitude,latitude,gc_test_3gcm,
               main=paste("Carte GC biais IPSL/MPI/CNRM",season,sep=" "),
               zlim=c(min(get(paste(domaine,"_biais_",season,sep=""))),max(get(paste(domaine,"_biais_",season,sep="")))),
               col=colorTable)
  lines(get(paste(domaine)))
  # mtext(get(paste(quant_nodes,side=1,line = 4)))
  ...
  dev.off()
  # setwd("/home/users/mgarvik/GraphCUT/multivarie")
}